...
前幾篇文章談的模型，不管是 MLP、CNN 還是 Transformer，都有一個大概前提：

> 你手上有一批資料，每筆資料都附有標籤。模型的任務，就是把輸入映射到正確的輸出；或至少你知道標準答案的大概分布是甚麼。
...
最後一列的內容是關鍵差異:
在 RL 裡，**你的選擇會改變你接下來看到的世界**。這讓整個問題的難度跳升了一個層次。
...
如果 observation 不等於 state，這個問題就叫 Partially Observable MDP (POMDP)，難度大幅上升。
這篇先假設 **fully observable**，讓問題可以處理。

### 先把時間索引說清楚

這篇文章沿用 RL 最常見的*記號*(Notation)：約定在時間 $t$ 做出動作 $a_t$ 之後，環境在下一個時間點回傳 reward $r_{t+1}$，並轉移到 $s_{t+1}$。

這樣寫不是多餘，而是為了讓 return 可以自然地寫成「現在這一步的立即回饋 + 未來所有回饋的折扣和」。換句話說，時間索引本身就已經在暗示 Bellman 分解會怎麼長。
...
這不是在說歷史不重要，而是在說：如果 state 定義得夠好，歷史的影響應該已經被壓縮進 state 裡了。
好的 state 設計，正是讓 Markov property 成立的關鍵。
...
如果是有限長的 episodic task，把上式改成

$$
G_t = \sum_{k=0}^{T-t-1} \gamma^k r_{t+k+1}
$$

其中 $T$ 是 episode 結束的時間點。無論是有限長或無限長設定，核心想法都一樣：我們不是只看下一步 reward，而是看「從現在開始一路往後」的累積效果。
...
把 $V^\pi$ 和 $Q^\pi$ 放在一起看，最常用的兩個等價關係是：
...
前者說明 $Q^\pi$ 可以直接拆成「立即 reward」和「下一個 state 的 value」；後者則說明 $V^\pi$ 是 policy 下對各個 action value 的期望。這兩個式子把整個 RL 的 value 系統連起來了。
...
接著把「對策略取期望」和「對環境轉移取期望」分開，就會得到：

$$
\begin{aligned}
V^{\pi}(s)
&= \sum_{a \in \mathcal{A}} \pi(a \mid s)
  \mathbb{E}\left[r_{t+1} + \gamma G_{t+1} \mid s_t=s, a_t=a\right] \\
&= \sum_{a \in \mathcal{A}} \pi(a \mid s)
...
整理後就是 Bellman expectation equation：
...
更精確地說，optimal policy 並不是先「找出一個最優 policy 再來評估」，而是先把所有未來影響壓進 $Q^*$，再在每個 state 上做局部最大化；Bellman optimality equation 保證這個局部最大化和全域最優是相容的。
...
> Insight：TD error 是整個 RL 演算法設計的共同語言。
> 不管是後來的 DQN、Actor-Critic 還是 PPO，它們的核心更新機制都是在處理「如何更穩定、更有效率地利用 TD error」這個問題。

Q-Learning 的一個漂亮性質：它是 **off-policy** 的。
也就是說，用來收集資料的行為策略（behavior policy），和正在被優化的目標策略（target policy），可以不同。這讓我們可以用歷史資料或探索性策略收集的樣本來更新 $Q^*$ 的估計。
