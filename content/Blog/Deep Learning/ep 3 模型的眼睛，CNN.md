...
> 這篇文章不只在說 CNN 怎麼算，而是在說為什麼影像任務需要卷積這種 inductive bias。
> 從 MLP 的限制出發，本文把卷積、ResNet 與 UNet 放在同一條敘事線上，說明它們各自解決的是哪一種結構問題。
...
在深度學習出現之前，影像分類是「手工特徵 + 傳統分類器」的流程：先跑 SIFT、HOG、顏色直方圖把影像壓成向量，再丟給 SVM 或 Logistic Regression 做分類。
這個流程的瓶頸不在分類器，而在特徵——不同任務要重新設計特徵，分類效果完全取決於設計者的直覺。

若把影像展平成向量後直接接上一層全連接層，假設輸入維度為 $D$、輸出維度為 $H$，那麼單層參數量就是

$$
DH + \underbrace{H}_{\text{bias}}
$$

對影像這種高維輸入而言，這個數字很快就會膨脹到難以承受。
...
**第三，沒有平移不變性。** 同一隻貓出現在圖片左上角或右下角，語意完全一樣，但對 MLP 來說它會學成兩組不同的參數。

這三個問題指向同一個結論：影像的問題不是「非線性不夠」，而是「歸納偏差 (inductive bias) 選錯了」。
MLP 把所有輸入位置都當成平等的、無結構的，但影像本來就有局部相關性、模式重複性，以及空間層次結構。

> 所以在圖像辨識的任務上，需要有一套能夠辨識局部特徵的設計解決上面三個問題 - CNN就這麼地被設計出來ㄌ

如果把影像看成函數 $x(u,v)$，那 MLP 等於把每個座標點都獨立看待；
CNN 則假設這個函數在局部區域內存在**可重複利用**的特徵。
...
### 示意圖
![[CNN.png|CNN視覺畫圖示]] 
MLP的問題是: 把所有像素當成獨立、無位置關係的數字。
CNN 的三個設計原則，剛好針對這三個缺點各自給出一個答案。



### 1. 局部連接（Local Connectivity）

**問題：MLP 讓每個神經元連到所有像素，太浪費了**

想像你在看一張臉。你辨識「眼睛」這個特徵時，看的是眼睛附近的那一小塊區域，而不是同時盯著臉的每一個角落。邊緣、角點、紋理，這些在影像中有意義的低階特徵，都是由鄰近像素之間的關係構成的，遠方的像素幾乎不提供任何幫助。

CNN 的做法：用一個小小的**卷積核（kernel）**，每次只看輸入的一小塊區域。

一個 $3 \times 3$ 的 kernel 只有 9 個參數，但它可以在整張圖上滑動，在每個位置做同樣的局部特徵偵測。相比之下，MLP 需要為每個輸入位置都學一套獨立的參數——一張 $224 \times 224$ 的影像就有 $150{,}528$ 個像素，參數量立刻爆炸。

**參數量的對比**

若輸入通道數為 $C_{in}$、輸出通道數為 $C_{out}$、kernel 大小為 $k \times k$，那麼一層卷積的參數量是：

$$k^2, C_{in}, C_{out} + C_{out}$$

關鍵在於：這個式子和影像的寬高完全無關。不管輸入是 $224 \times 224$ 還是 $1080 \times 1920$，卷積層的參數量都一樣，只由 kernel 大小與 channel 數決定。

### 2. 權重共享（Weight Sharing）

**問題：同一個特徵出現在不同位置，不應該學兩套參數**

「水平邊緣」不管出現在圖片左上角還是右下角，在視覺上都是同一種特徵。
但 MLP 沒有這個概念——如果貓換個位置，它會把這當成完全不同的輸入，需要重新學習。

CNN 的做法：**同一組 kernel 在所有位置重複使用**。

這帶來了一個重要的數學性質——**平移等變性（translation equivariance）**：如果輸入往右平移了幾格，輸出的 feature map 也會跟著往右平移對應的格數，而不是輸出完全不同的結果。
用數學算是來說：

$$\mathcal{C}_K(T_\Delta X) = T_\Delta(\mathcal{C}_K(X))$$

其中 $T_\Delta$ 是平移運算，$\mathcal{C}_K$ 是卷積運算。
這個等式的意思就是「先平移再卷積」等於「先卷積再平移」——同一個特徵不管在哪裡，都會被同一組參數偵測到。

後續加上的池化（pooling）操作，則進一步把等變性轉成近似的**不變性（invariance）**：小幅度的位置偏移不會改變最終輸出，讓模型對微小的位移、形變更有容忍度。

### 3. 層級特徵組合（Hierarchical Feature Composition）

**問題：單層卷積的*感受野*(Receptive field)太小，看不到大範圍的結構**

~~感受野到底是甚麼鬼翻譯~~

一個 $3 \times 3$ 的 kernel 每次只看 9 個像素。
靠這麼小的視野，能學到的頂多是局部邊緣或顏色漸變，沒辦法理解「鼻子」或「耳朵」這種需要整合大片區域的概念。

CNN 的做法：**把多層卷積堆疊起來**，讓下層學到的簡單特徵，在上層被組合成更複雜的模式。

- 低層（Layer 1）：邊緣、方向、顏色漸變
- 中層（Layer 2~3）：角點、紋理、曲線
- 高層（Layer N）：眼睛、鼻子、整張臉

> 這個「從簡單到抽象」的層級結構，和人類視覺皮層的運作方式非常接近。
> Hubel 與 Wiesel 在 1960 年代的研究就發現，視覺皮層裡的神經元只對局部感受野（receptive field）內的刺激有反應，而且不同神經元對不同方向的邊緣敏感——CNN 幾乎是把這個神經科學發現直接在模型設計上復刻了。

**感受野如何隨深度擴大**

若每層都是 $3 \times 3$、stride=1 的卷積，那麼第 $L$ 層的感受野大小大約是：
...
這說明三層 $3 \times 3$ 卷積的等效感受野和一層 $7 \times 7$ 相同——但後者的參數量為49， 對比前者只有$3 \times 9 = 27$ ，還多了兩次非線性變換，表達能力更豐富。
> 這正是後續介紹的 VGG 等架構傾向選擇「用小 kernel 堆深層」的核心原因。
...
> 一個常見誤解：深度學習框架裡的「卷積」，其實是 cross-correlation，不是數學上嚴格的卷積。
> **差別在於 kernel 有沒有先翻轉。**
...
因為 kernel 是訓練出來的，翻不翻轉只是學到的 kernel 長得不一樣，對模型能力沒有影響。所以兩者混用在實務上問題不大。

若輸入高寬為 $H\times W$、kernel 為 $k\times k$、stride 為 $s$、padding 為 $p$，則輸出高寬為
...
這是後面追蹤 tensor shape 時最常用的公式。

### 多通道卷積

RGB 影像不是二維，而是三維張量。
一個輸出通道對應一組 kernel，這組 kernel 會跨過所有輸入通道做加總：
...
這裡最容易搞混的是：一個輸出通道不只看一個輸入通道，它會**同時整合所有輸入通道的訊號**，再加總成一個 feature map。

換句話說，每個輸出通道都對應一個跨通道濾波器，channel 維度是卷積運算的一部分，而不是附帶資訊。

這段內容的問題很清楚：三個區塊（Tensor flow、卷積變體、池化與非線性）各說各話，讀者讀完第一段不知道為什麼要接下去看第二段。

改寫的核心策略是：**用一條「從實作場景出發，逐步解釋為什麼需要這些工具」的敘事線，把三個區塊串成一個完整的推進過程**。

---

## 從一個完整的 CNN 看起

> 接下來我們從一個CNN的模型架看起，並逐一介紹在convolution以外的設計

理解個別操作之前，先把整體流程看一遍。
以一個典型的影像分類網路為例，資料從輸入到輸出的 tensor 變化長這樣：

```
輸入         [B, 224, 224, 3]
Conv(64)     [B, 224, 224, 64]    ← same padding，保持空間尺寸
MaxPool      [B, 112, 112, 64]    ← 2×2, stride 2，尺寸減半
...
這張流程圖裡有兩個反覆出現的節奏：**Conv 負責抽特徵、MaxPool 負責縮空間**，交替進行，最後用 Global Average Pooling 把空間維度壓掉，留下純粹的語意向量再送入分類器。

只看這張圖的架構，很自然地會有幾個問題：
- Conv 要怎麼設計才能控制輸出的大小？
- MaxPool 的「縮空間」到底在做什麼？
- 每個單元內部又是怎麼運作的？
...
## 控制輸出大小：Stride、Padding 與 Dilation

卷積輸出的空間尺寸，由三個*超參數*(Hyperparmeter)共同決定。

**Padding：決定邊緣要不要補零**

不補零時，kernel 每滑過一次都會讓邊緣縮水，輸出尺寸是 $H_{out} = H - k + 1$，這叫 valid convolution。
補零讓輸出維持原本的大小，$H_{out} = H$，叫 same convolution——這也是上面的流程圖裡 `Conv(64)` 之後尺寸沒有變小的原因。
還有一種補更多零讓輸出反而比輸入更大的 full convolution，稍後講到轉置卷積時會再提到。

**Stride：決定 kernel 每次跳多遠**

若 stride=2，kernel 每次跳兩格，輸出尺寸直接減半。
這件事和 MaxPool 的效果很像，但好處是**特徵提取和下採樣可以合在一步完成**，不需要分開做。
現代架構（如 ResNet）傾向用 stride convolution 取代 MaxPool，減少中間計算量。

**Dilation：在不增加參數的情況下擴大感受野**

> murmur: 這邊好難，日後再另開一篇深入研究

上述兩個參數都是針對「滑動方式」做調整。


Dilation 則是在 kernel 內部插入間隔：dilation rate $d=2$ 時，原本相鄰的 kernel 元素之間會空出一格，讓 $3 \times 3$ 的 kernel 實際覆蓋 $5 \times 5$ 的範圍，但參數量完全不變。

$$Y(i, j) = \sum_{m}\sum_{n} X(i + d \cdot m, j + d \cdot n), K(m, n)$$

一般來說，dilation rate $d$ 時，有效感受野從 $k$ 擴大到 $k + (k-1)(d-1)$。
DeepLab 系列大量使用這個技巧：在語義分割任務裡，如果用 pooling 縮小空間尺寸，後來再做上採樣時細節就回不來了；改用 dilated convolution，可以在保持空間解析度的同時，讓感受野一樣夠大。

---

## 池化：壓縮空間，保留「有沒有」而非「在哪裡」

回到流程圖裡的 MaxPool，它做的不只是縮小尺寸。

Max pooling 在每個局部視窗 $\Omega_{ij}$ 裡取最大值：

$$h_{out}(i,j) = \max_{(u,v) \in \Omega_{ij}} h_{in}(u,v)$$

最大值的意義是「這個區域裡**有沒有**某種特徵存在」。
只要特徵出現在視窗內的任何位置，最大值就能把它保留下來。
這讓模型對微小的位置偏移天然具有容忍度——特徵往左移一格，最大值不變，輸出也不變，也就是CNN**局部不變性**的來源。

Average pooling 取平均，語意則換成「這個區域的特徵有多強」，更像是做一個全局強度的統計摘要。
兩者哪個更好，取決於任務：辨識任務關心「有沒有」，所以 max pooling 通常更有效；最後收尾用的 Global Average Pooling（GAP）則是把整張 feature map 壓成一個數字，做的是全局統計，用的是 average 的邏輯。

---

## 每個卷積單元內部：Conv → BN → ReLU

上面講的都是「層與層之間怎麼接」，現代 CNN 對每一層內部的結構也有幾乎固定的慣例：先做卷積，再做 Batch Normalization，最後接 ReLU。

**Conv** 完成線性的局部特徵抽取。輸出是一個加權疊加，本身沒有非線性。

**Batch Normalization（BN）** 接著把這一批樣本在某個 channel 上的 activation 拉回到均值 0、方差 1 附近。具體地，對 batch 中的 activation $x_1, \dots, x_m$：
...
正規化之後，還會乘上一組可學習的縮放與平移參數 $\gamma, \beta$，最終輸出是 $\gamma \hat{x}_i + \beta$。這也是 BN 不只是「把數值壓到 $[-1, 1]$」的原因——它保留了模型重新學習合適分佈的彈性。

直覺上，BN 的作用是把損失**地形**「修平」：各層的輸入分佈變得更穩定，梯度不容易在穿越很多層之後就消失或爆炸。

**ReLU** 最後引入非線性。沒有它，不管堆多少層 Conv 和 BN，整個網路仍然只是一個線性變換——多層沒有帶來額外的表達能力。

三者合在一起，Conv 決定「看什麼」，BN 決定「數值穩不穩」，ReLU 決定「要不要激活」，形成現代 CNN 每個 block 的最小可重複單元。
...
AlexNet 在 ImageNet 上把 top-5 錯誤率從 26% 降到 15%，直接引爆深度學習熱潮。它的關鍵貢獻是把 ReLU、Dropout、資料擴增(Data augumentation)、多 GPU 訓練這些實作技巧全部組合在一起，驗證深層 CNN 在大規模資料上的有效性。

### VGGNet（2014）

VGG 的策略非常簡單：只用 $3 \times 3$ 卷積，然後不斷堆疊。
VGG-16 有 16 層，VGG-19 有 19 層。
它的貢獻是確立了「深度很重要，kernel 要小」的設計原則，但參數量仍然巨大（138M），推理速度也慢。
...
直覺上，網路越深，表達能力越強，至少理論上不應該比淺層的版本更差。
但實驗結果告訴我們，56 層的普通 CNN 在訓練集上的錯誤率比 20 層更高。
這不是 overfitting（測試集也更差），而是優化問題——深層網路在訓練時根本學不到有效的東西。

**原因出在梯度**。
網路太深時，梯度從 loss 往回傳，每經過一層都要乘上一個數，如果這個數小於 1，乘到最後梯度就接近 0，底層的參數完全學不動，這就是**梯度消失（Vanishing Gradient）**。
...
那麼反向傳播時會連乘許多個 $g'(\cdot)$ 和權重矩陣。
當這些 Jacobian (指權重矩陣的微分後結果(梯度) )長期小於 1 時，梯度就會指數式衰減。
...
為什麼這樣有用？因為如果這組層什麼都不學（$F(x) = 0$），輸出直接就是恆等映射 $H(x) = x$。
這個「什麼都不做」的極端情況對一般 CNN 來說很難學到，但對 ResNet 來說只需要把 $F(x)$ 學成 0 就好了。
換句話說，**學殘差比學完整映射更容易初始化到一個合理的起點**。

更關鍵的是梯度流動。Skip connection 提供了一條梯度的高速公路，讓梯度可以直接從後面的層流回到前面的層，完全不衰減：

$$
\frac{\partial \mathcal{L}}{\partial x} = \frac{\partial \mathcal{L}}{\partial H} \cdot \frac{\partial H}{\partial x} = \frac{\partial \mathcal{L}}{\partial H} \cdot \left(\mathbf{1} + \frac{\partial F}{\partial x}\right)
$$

就算 $\frac{\partial F}{\partial x}$ 很小，加上 1 之後梯度也不會消失。

這個式子就是 ResNet 能訓練更深的關鍵：identity path 提供了一條不需要經過層內非線性的直通路徑。
...
Basic Block：
  ...
  Conv(3×3, stride=1, padding=1) → BN + skip connection → ReLU
```
...
那麼 identity shortcut 就是 $W_s=I$，projection shortcut 則是用可訓練的線性映射把維度對齊。
...
分類只問「這張圖裡有什麼？」，一個標籤就夠了。
分割問的是「每個像素屬於哪個類別？」，輸出尺寸要和輸入完全一樣。
...
$$
f: \mathbb{R}^{H\times W\times C} \to \mathbb{R}^{H\times W\times K}
$$

也就是說，輸出不再是一個 class vector，而是每個像素都要有一個 class distribution。
...
> Note：原版 UNet 用 valid convolution，所以空間尺寸在每個 conv 後都縮小 2。
> 如果改用 same convolution + padding，可以保持尺寸不變，實作更簡單，現代實作多半這樣做。
...
Transposed Convolution（有時誤稱 deconvolution）是 UNet 做上採樣的核心機制。
它的作用是把每個輸入像素展開，乘上 kernel，在輸出空間上疊加。
...
另一個常見替代方案是 **Bilinear Upsampling + Conv**：先用雙線性插值把尺寸放大，再接一個普通卷積調整特徵。
這種做法更穩定，不容易出現 transposed conv 的棋盤格 artifact。

如果把 transposed convolution 的輸出尺寸公式寫完整，則

$$
H_{out} = (H_{in} - 1) \cdot s - 2p + k + o
$$

其中 $s$ 是 stride、$p$ 是 padding、$k$ 是 kernel size、$o$ 是 output padding。這也是實作時最常見的 shape mismatch 來源。
...
卷積層與池化層的輸出空間尺度 $O$ 計算公式為：

$$O = \lfloor \frac{W - K + 2P}{S} \rfloor + 1$$

- $W$：輸入特徵圖尺寸 (Input Size)
    
- $K$：卷積核或池化核尺寸 (Kernel Size)
    
- $P$：填充 (Padding)
    
- $S$：步幅 (Stride)
    
- $\lfloor \cdot \rfloor$：向下取整 (Floor operation)
    

反卷積層 (Transposed Convolution，用於上採樣) 的輸出空間尺度 $O$ 計算公式為：

$$O = (W - 1) \times S - 2P + K + OP$$

- $OP$：輸出填充 (Output Padding，通常預設為 0)
    

### ResNet-34 尺度變化分析

ResNet 架構利用步幅 $S=2$ 的卷積層進行下採樣，而非全數依賴池化層。以輸入尺寸 $224 \times 224$ 為例：

1. **Conv1**: $K=7, S=2, P=3$
    
    - $$O = \lfloor \frac{224 - 7 + 2(3)}{2} \rfloor + 1 = 112$$
        
    - 輸出：$112 \times 112$
        
2. **MaxPool**: $K=3, S=2, P=1$
    
    - $$O = \lfloor \frac{112 - 3 + 2(1)}{2} \rfloor + 1 = 56$$
        
    - 輸出：$56 \times 56$
        
3. **Layer 1 (Conv2_x)**: $K=3, S=1, P=1$
    
    - 尺度不變。輸出：$56 \times 56$
        
4. **Layer 2 (Conv3_x)**: 區塊首層 $K=3, S=2, P=1$
    
    - $$O = \lfloor \frac{56 - 3 + 2(1)}{2} \rfloor + 1 = 28$$
        
    - 輸出：$28 \times 28$
        
5. **Layer 3 (Conv4_x)**: 區塊首層 $K=3, S=2, P=1$
    
    - $$O = \lfloor \frac{28 - 3 + 2(1)}{2} \rfloor + 1 = 14$$
        
    - 輸出：$14 \times 14$
        
6. **Layer 4 (Conv5_x)**: 區塊首層 $K=3, S=2, P=1$
    
    - $$O = \lfloor \frac{14 - 3 + 2(1)}{2} \rfloor + 1 = 7$$
        
    - 輸出：$7 \times 7$
        
7. **Global Average Pooling**:
    
    - 將空間維度直接壓縮。輸出：$1 \times 1$
        

### U-Net 尺度變化分析

原始 U-Net 論文使用**無填充卷積 (Valid Convolution, $P=0$)**，導致每次卷積操作後特徵圖尺寸縮減。

1. **下採樣路徑 (Encoder)**:
    
    - 輸入：$572 \times 572$
        
    - 連續兩次 Conv ($K=3, S=1, P=0$):
        
        - 第一次：
            
            $$O = \frac{572 - 3 + 0}{1} + 1 = 570$$
            
        - 第二次：
            
            $$O = \frac{570 - 3 + 0}{1} + 1 = 568$$
            
    - MaxPool ($K=2, S=2$):
        
        - $$O = \lfloor \frac{568 - 2}{2} \rfloor + 1 = 284$$
            
2. **上採樣路徑 (Decoder) 與跳接 (Skip Connection)**:
    
    - 使用 Transposed Conv ($K=2, S=2$) 將特徵圖長寬放大 2 倍。
        
    - **特徵拼接 (Concatenation) 的尺度對齊**：由於編碼器層的輸出大於對應解碼器層的輸入（因 $P=0$ 縮減），必須對編碼器特徵圖進行**中心裁切 (Center Cropping)**。
        
    - 範例：解碼器當前尺寸為 $56 \times 56$，對應的編碼器特徵圖為 $64 \times 64$。必須將 $64 \times 64$ 裁切為 $56 \times 56$ 後，再與解碼器特徵圖在通道維度上拼接。
        

_(註：現代 U-Net 實作多數採用 Same Padding ($P=1$)，使卷積不改變空間尺寸，從而免除跳接時的裁切操作。)_
...
**R-CNN 系列**的演化主線是把重複計算消掉。
R-CNN 每個候選區域跑一次 CNN，太慢；Fast R-CNN 先跑整張圖，再用 RoI Pooling 從 feature map 截取區域；Faster R-CNN 把候選區域的生成（Region Proposal Network）也學進模型裡，整個流程端到端。
