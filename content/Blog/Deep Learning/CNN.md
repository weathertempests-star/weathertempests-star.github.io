...
# CNN：影像任務為什麼需要卷積、殘差與 U-Net？

> 這篇文章不是只在說 CNN 怎麼算，而是在說為什麼影像任務需要卷積這種 inductive bias。
...
如果把影像看成函數 $x(u,v)$，那 MLP 等於把每個座標點都獨立看待；CNN 則假設這個函數在局部區域內存在可重複利用的模式，這正是卷積能工作的原因。
...
若輸入通道數為 $C_{in}$、輸出通道數為 $C_{out}$、kernel 大小為 $k\times k$，那麼卷積層的參數量是

$$
k^2 C_{in} C_{out} + C_{out}
$$

這個式子直接點出 CNN 的效率來源：參數量與影像寬高無關，只和 kernel 尺寸與 channel 數有關。
...
如果把卷積記成運算子 $\mathcal{C}_K$，則平移運算 $T_\Delta$ 可以寫成近似關係：

$$
\mathcal{C}_K(T_\Delta X) = T_\Delta(\mathcal{C}_K(X))
$$

這就是卷積「在哪裡看到同一個特徵，輸出就在哪裡反映出來」的數學版本。
...
從感受野角度看，若每層都是 $3\times 3$、stride=1 的卷積，且暫時忽略邊界效應，那麼第 $L$ 層的感受野大小可以近似寫成

$$
R_L = 2L + 1
$$

也就是說，三層 $3\times 3$ 卷積大致對應 $7\times 7$ 的感受野，但中間多了兩次非線性，參數也更少。
...
若把 pooling 看成局部函數，max pooling 可寫成

$$
h_{out}(i,j)=\max_{(u,v)\in \Omega_{ij}} h_{in}(u,v)
$$

其中 $\Omega_{ij}$ 是局部視窗。這也說明它為什麼能帶來局部不變性。
...
若 batch 中某個 channel 的 activation 為 $x_1,\dots,x_m$，則 BN 先算
...
最後再乘上可學的縮放與平移參數。這就是 BN 為什麼不只是「正規化」，而是可學的特徵重標定。
...
那麼反向傳播時會連乘許多個 $g'(\cdot)$ 和權重矩陣。當這些 Jacobian 長期小於 1 時，梯度就會指數式衰減。
