...
因為 KL divergence $\ge 0$，所以 ELBO 永遠是 $\log p_\theta(x)$ 的下界。當 $q_\phi = p_\theta(z \mid x)$ 時等號成立。
...
= -\frac{1}{2\sigma_x^2}\mathbb{E}_{q_\phi}\!\left[\|x - f_\theta(z)\|^2\right] + \text{const}
$$
...
令 $q = \mathcal{N}(\mu_q, \operatorname{diag}(\sigma_q^2))$，$p = \mathcal{N}(0, I)$（標準 Gaussian prior），則：
...
那梯度就在這個隨機節點卡住，沒辦法傳回 encoder。

Reparameterization Trick 把 randomness 從 $z$ 挪到一個跟參數無關的 $\epsilon$：

$$
z = \mu_\phi(x) + \sigma_\phi(x) \odot \epsilon, \qquad \epsilon \sim \mathcal{N}(0, I)
$$

現在 $z$ 是 $\mu_\phi$ 和 $\sigma_\phi$ 的確定性函數，$\epsilon$ 是外部的隨機噪聲。梯度可以順著 $\mu_\phi$ 和 $\sigma_\phi$ 一路傳回 encoder：

$$
\nabla_\phi\, \mathbb{E}_{q_\phi(z \mid x)}[f(z)]
=
\nabla_\phi\, \mathbb{E}_{\epsilon \sim \mathcal{N}(0,I)}
\!\left[f\!\left(\mu_\phi(x) + \sigma_\phi(x) \odot \epsilon\right)\right]
$$

右邊的期望對 $\phi$ 求梯度，直接穿過 $\mu_\phi$ 和 $\sigma_\phi$ 兩個神經網路，backpropagation 正常運作。
