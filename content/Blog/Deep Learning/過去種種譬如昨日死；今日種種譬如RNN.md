...
> RNN 的核心不是把模型做得更深，而是把「序列」改寫成「狀態反覆更新」的問題。
> 本文從 MLP 的限制出發，推到 vanilla RNN 的遞迴形式、BPTT 的梯度連乘，最後說明 LSTM 為什麼能把記憶變得可控。
...
從系列脈絡看，這一篇承接前面的 MLP、CNN，進一步回答的是：當資料不是固定長度的影像，而是有順序、有上下文依賴的序列時，模型的表示方式應該怎麼改。

---

## 為什麼 MLP 不適合序列資料？

MLP 的輸入是一個固定長度的向量，所有位置都被當成**平等、無順序**的特徵。
但序列資料根本不長這樣。

### 序列資料的三個本質特性

1. **順序本身就是資訊:** 「狗咬人」和「人咬狗」用的是同樣的詞，但意思截然不同。如果把 token 展平成向量再丟進 MLP，順序就消失了。

2. **輸入長度是變動的:** MLP 的輸入維度是固定的，但句子、音訊、時間序列，長度在訓練和測試時都可能不一樣。

3. **當前位置的意義依賴前文。** 閱讀一個詞時，你不只看這個詞本身，還要記住前面讀過什麼。MLP 沒有跨位置的「記憶」機制。

這三個問題都指向同一件事：MLP 把每筆輸入看成**獨立、無結構的點**，但序列資料需要的是**有記憶的、帶時間感的閱讀方式**。

> **Insight：** MLP 的弱點不是表達能力不夠，而是歸納偏差（inductive bias）選錯了。
> 影像的問題是空間局部性，序列的問題是時間依賴性。
> CNN 解決前者，RNN 解決後者。

如果把序列 $x^{(1)}, x^{(2)}, \dots, x^{(T)}$ 看成一個時間索引的函數，那麼 MLP 要做的是把整段序列一次性映射到輸出；RNN 則是把這件事拆成多個時間步，讓中間狀態 $h^{(t)}$ 成為「目前為止讀到的摘要」。
...
其中 $h^{(0)}$ 通常設成零向量或可學的初始向量。這個遞迴式表示：第 $t$ 步的狀態不只依賴當前輸入，也依賴前一時刻的記憶。
...
> **Warning：** 「RNN 可以處理任意長度的序列」是一個常被誤解的說法。
> 它的確可以逐步讀入任意長的輸入，但 hidden state 的維度是固定的，所以長序列仍然會有信息丟失。
> 能處理和能記住，是兩件不同的事。
...
$$
h^{(t)} = \tanh\!\bigl(b + W h^{(t-1)} + U x^{(t)}\bigr)
$$

這是 vanilla RNN 最常見的寫法: 用線性變換把舊記憶和當前輸入混合，再用非線性把狀態壓回固定維度的記憶空間。
...
這個式子的意思是：第 $t$ 步的誤差，不只來自自己當步的輸出，也會收到未來時間步透過 recurrent connection 傳回來的影響。
因為同一個 $W$ 被重複使用，所以每往回傳一個時間步，就多乘一次 $W^T$ 與激活函數導數。
...
這三個式子都在提醒一件事：RNN 的參數是共享的，所以梯度不是某一層自己的，而是整段序列上所有時間步貢獻的總和。這也是 BPTT 與一般 MLP backprop 最大的不同。
...
更精確地說，若 Jacobian 的譜範數長期小於 1，梯度就會隨著時間距離指數衰減；若大於 1，就會指數放大。這就是為什麼 RNN 對長距離依賴特別敏感。
...
如果把 hidden state 當成一個累積摘要，中心化的輸出意味著更新時比較容易同時表達正向與負向資訊，這也是 `tanh` 在早期 RNN 中比 sigmoid 更常被使用的原因。
...
Forget gate 的輸出介於 0 和 1 之間；若某個維度接近 1，表示幾乎完全保留舊記憶，若接近 0，表示幾乎完全忘掉。
...
這條式子是 LSTM 最重要的地方:
把「記憶保留」和「新資訊寫入」拆成兩個可控的乘法掩碼，讓模型能夠選擇性地更新 state，而不是被動地讓所有資訊一起混進去。
...
Output gate 決定的是「現在要對外公開多少內部記憶」。cell state 負責長期保存，hidden state 負責對外輸出，兩者在設計上被刻意分工。
...
這也是 LSTM 能緩解梯度消失的核心：時間軸上的主通道變成加法累積，而不是純粹的矩陣連乘，因此梯度不再只能靠不斷乘 Jacobian 傳遞。
...
真實序列可能非常長（例如整篇文章），把梯度一路傳回去計算成本極高。
**Truncated BPTT** 的做法是把序列切成若干段，每段只做有限步的反向傳播，但 hidden state 的值在段與段之間保留下來繼續傳遞。

這是計算效率和梯度正確性之間的妥協。

如果把完整 BPTT 的時間長度記成 $T$，截斷長度記成 $\tau$，那麼每次反向只傳遞最近 $\tau$ 步的梯度。這樣做會引入近似，但可以把訓練成本從完整長序列的爆炸級計算量，降到可實作的範圍。
...
若把 encoder 的最後狀態記為 $z$，decoder 則是在做

$$
p(y^{(1:T')}\mid x^{(1:T)}) = \prod_{t=1}^{T'} p(y^{(t)} \mid y^{(<t)}, z)
$$

這個分解再次凸顯 autoregressive 的本質：每一步都依賴前一步的輸出與共享的 context。
