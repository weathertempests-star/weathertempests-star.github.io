...
這是 vanilla RNN 最常見的寫法。它的本質是：用線性變換把舊記憶和當前輸入混合，再用非線性把狀態壓回固定維度的記憶空間。
...
這個式子的意思是：第 $t$ 步的誤差，不只來自自己當步的輸出，也會收到未來時間步透過 recurrent connection 傳回來的影響。因為同一個 $W$ 被重複使用，所以每往回傳一個時間步，就多乘一次 $W^T$ 與激活函數導數。
...
這條式子是 LSTM 最重要的地方。它把「記憶保留」和「新資訊寫入」拆成兩個可控的乘法掩碼，讓模型能夠選擇性地更新 state，而不是被動地讓所有資訊一起混進去。
