...
未完的繁體中文技術閱讀與學習紀錄。文章依主題標籤探索，筆記沿系列篇章深入。

網站使用 Astro、TypeScript、Markdown、KaTeX 與 Pagefind，輸出靜態檔案並部署到 GitHub Pages。包含首頁、文章、系列筆記、關於、全文搜尋、RSS、sitemap 與 404；關於頁說明虛擬作者與人機共創方式。
...
- `src/content/blog/`：獨立文章與論文導讀，透過主題標籤探索相關內容。
- `src/content/notes/`：依系列與篇次管理的筆記，目前包含 OP-TEE 首篇。
- `src/data/series.ts`：系列名稱、介紹與狀態；沒有已發布篇章的系列不產生公開頁面。
...
十篇論文文章已移入 blog，原 blog 文章及示範草稿已移除。舊論文 note 網址以轉址頁導向新文章，並從搜尋、RSS 與 sitemap 排除；OP-TEE 保留原 note 網址。
...
blog 可選填 `paper` 物件，提供時以下欄位須完整；一般文章不必填寫。範例：
...
文章的 `date` 是文章發表日；`paper.published` 是預印本首次提交日或正式論文發表日。首頁、列表及 RSS 依文章日期排列，來源框另外顯示論文日期及閱讀版本。

論文導讀以「Agent 系統」「模型訓練」「AI 安全」「機密運算」其中一個主題作為首個標籤，再加必要技術標籤。正文約 1,200–2,000 中文字，從問題解釋方法、證據與限制；引用放在相關敘述旁，原論文結果與編者推論應分清楚。

### 系列筆記與篇次

先在 `src/data/series.ts` 登記穩定的系列 id、名稱、介紹及狀態，再在 notes frontmatter 加入：
...
`series` 必填，id 必須存在於系列目錄，order 必須是正整數。同一系列的公開篇次不可重複，重複會令建置失敗。notes 不需要 `paper`。

`/notes/` 顯示有公開篇章的系列，`/notes/series/<id>/` 依 order 排序。文章內的系列目錄、上一篇與下一篇只連到已發布內容；日期不影響系列順序。只有一篇時不顯示不存在的上下篇。草稿不列入篇數、閱讀時間、目錄或導覽。

`/blog/?tag=...` 保存文章標籤選擇，支援重新整理與上一頁；文章頁標籤和相關文章提供延伸閱讀。搬移網址的清單位於 `src/data/migrations.ts`，轉址只會為仍已發布的目標文章產生。
